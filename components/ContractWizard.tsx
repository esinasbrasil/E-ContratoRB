...
const ContractWizard: React.FC<ContractWizardProps> = ({ 
  ...
  const handleSaveDraft = async () => {
    if (!formData.supplierId) { alert("Selecione um fornecedor para salvar."); return; }
  ...
  const renderStepContent = () => {
    switch (currentStep) {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center gap-4 p-5 border-2 rounded-2xl hover:bg-gray-50 cursor-pointer transition-all">
              ...
              <div><span className="block text-sm font-bold text-gray-800">Possui Contrato Social?</span><span className="text-[10px] text-gray-400">Verifique se está assinado</span></div>
            </label>
            <label className="flex items-center gap-4 p-5 border-2 rounded-2xl hover:bg-gray-50 cursor-pointer transition-all">
              ...
              <div><span className="block text-sm font-bold text-gray-800">Possui Pesquisas Serasa?</span><span className="text-[10px] text-gray-400">Certidões e consultas financeiras</span></div>
            </label>
          </div>
        </div>
      );
            ...
            {formData.prepostos.map((p, i) => (
              <div key={i} className="p-6 border-2 border-gray-100 rounded-[2rem] bg-slate-50 relative group transition-all hover:border-primary-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Nome Completo</label><input className="w-full p-3 text-sm border-0 rounded-xl bg-white shadow-sm" value={p.name} onChange={e => { const n = [...formData.prepostos]; n[i].name = e.target.value; handleChange('prepostos', n); }} /></div>
                  <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Cargo</label><input className="w-full p-3 text-sm border-0 rounded-xl bg-white shadow-sm" value={p.role} onChange={e => { const n = [...formData.prepostos]; n[i].role = e.target.value; handleChange('prepostos', n); }} /></div>
                  <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">CPF</label><input className="w-full p-3 text-sm border-0 rounded-xl bg-white shadow-sm" value={p.cpf} onChange={e => { const n = [...formData.prepostos]; n[i].cpf = e.target.value; handleChange('prepostos', n); }} /></div>
                  <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">E-mail</label><input className="w-full p-3 text-sm border-0 rounded-xl bg-white shadow-sm" value={p.email} onChange={e => { const n = [...formData.prepostos]; n[i].email = e.target.value; handleChange('prepostos', n); }} /></div>
                </div>
              </div>
            ))}
          </div>
      ...
      case 4: return (
        <div className="space-y-6">
          <h3 className="text-lg font-bold text-gray-800">5. Recursos Envolvidos</h3>
          <div className="grid grid-cols-1 gap-6">
            {[
              { id: 'hasMaterials', list: 'materialsList', label: 'Materiais' },
              { id: 'hasEquipment', list: 'equipmentList', label: 'Equipamentos' },
              { id: 'hasRental', list: 'rentalList', label: 'Locação' },
              { id: 'hasComodato', list: 'comodatoList', label: 'Comodato' }
            ].map(item => (
              <div key={item.id} className={`p-6 border-2 rounded-[2rem] transition-all duration-300 ${ (formData as any)[item.id] ? 'bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-50' : 'bg-white border-gray-100 hover:border-emerald-100'}`}>
                <label className="flex items-center gap-4 cursor-pointer">
                  ...
                  <span className="text-sm font-black text-gray-700 uppercase tracking-widest">Inclui {item.label}?</span>
                </label>
                {(formData as any)[item.id] && (
                  <div className="mt-4 animate-in slide-in-from-top-2 duration-300">
                    <label className="block text-[8px] font-black text-emerald-600 uppercase mb-1">Descreva os itens abaixo:</label>
                    <textarea placeholder={`Liste os ${item.label.toLowerCase()} envolvidos...`} className="w-full p-4 text-sm border-0 rounded-2xl bg-white shadow-inner focus:ring-2 focus:ring-emerald-500" rows={3} value={(formData as any)[item.list]} onChange={e => handleChange(item.list as any, e.target.value)} />
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 bg-emerald-50 rounded-[2.5rem] border-2 border-emerald-100">
               <label className="block text-[10px] font-black text-emerald-600 uppercase mb-2">Valor Total Estimado (R$)</label>
               <input type="number" className="w-full p-2 text-2xl font-black text-emerald-700 bg-transparent border-0 focus:ring-0" value={formData.value} onChange={e => handleChange('value', parseFloat(e.target.value))} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-slate-50 rounded-2xl">
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Data Início</label>
                <input type="date" className="w-full p-2 bg-transparent border-0 text-sm font-bold" value={formData.startDate} onChange={e => handleChange('startDate', e.target.value)} />
              </div>
              <div className="p-4 bg-slate-50 rounded-2xl">
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Data Término</label>
                <input type="date" className="w-full p-2 bg-transparent border-0 text-sm font-bold" value={formData.endDate} onChange={e => handleChange('endDate', e.target.value)} />
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {legalChecklist.map(item => (
                <label key={item.id} className={`flex items-start gap-4 p-4 border rounded-2xl transition-all cursor-pointer ${ (formData as any)[item.id] ? 'bg-white border-blue-400 shadow-md' : 'bg-white/50 border-blue-100 hover:bg-white'}`}>
                  <input type="checkbox" className="h-5 w-5 mt-1 text-blue-600 rounded border-gray-300" checked={(formData as any)[item.id]} onChange={e => handleChange(item.id as any, e.target.checked)} />
      ...
      case 8: return (
        <div className="space-y-6">
          <div className="flex justify-between items-center"><div><h3 className="text-lg font-bold text-gray-800">9. Documentos Anexos</h3><p className="text-xs text-slate-500 font-medium">Arquivos em PDF.</p></div><div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-full border border-emerald-100"><FileCheck size={16}/><span className="text-xs font-black">{formData.attachments.length} Arquivos</span></div></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto max-h-[450px] pr-2 scrollbar-thin">
            {attachmentTypes.map(type => {
              ...
              return (
                <div key={type} className={`p-5 border-2 rounded-[2rem] flex flex-col justify-between min-h-[110px] transition-all group ${hasFile ? 'bg-emerald-50/50 border-emerald-200' : 'bg-white border-slate-100 hover:border-primary-200 shadow-sm'}`}>
                  <div className="flex justify-between items-start gap-4 mb-3"><span className={`text-[10px] font-black uppercase tracking-tight leading-tight ${hasFile ? 'text-emerald-700' : 'text-slate-500'}`}>{type}</span>{hasFile ? <Check className="text-emerald-500 shrink-0" size={18} /> : <div className="w-5 h-5 rounded-full border-2 border-slate-100 shrink-0" />}</div>
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-[10px] text-slate-400 font-bold truncate max-w-[150px]">{hasFile ? attachment.name : 'Vazio'}</span>
                    <div className="flex gap-2">
                      {hasFile && <button onClick={() => handleChange('attachments', formData.attachments.filter(a => a.type !== type))} className="p-2 text-red-400 hover:text-red-600 rounded-xl transition-all"><X size={14} /></button>}
                      <label className="p-2.5 rounded-xl cursor-pointer bg-slate-50 text-slate-400 hover:bg-primary-600 hover:text-white transition-all"><Upload size={16} /><input type="file" className="hidden" accept=".pdf" onChange={e => handleFileUpload(e, type)} /></label>
                    </div>
                  </div>
                </div>
              );
      ...
      case 9: return (
        <div className="space-y-8">
          <div className="flex items-center gap-6 p-8 bg-emerald-50 text-emerald-700 rounded-[3rem] border border-emerald-100 shadow-inner">
             ...
             <div><h3 className="text-2xl font-black tracking-tighter">Revisão Final</h3><p className="text-sm font-medium text-emerald-600">Verifique os dados antes de finalizar.</p></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-gray-100 flex flex-col h-[92vh] w-full max-w-6xl mx-auto overflow-hidden animate-in zoom-in-95 duration-500">
      <div className="px-12 py-8 border-b border-gray-50 flex justify-between items-center sticky top-0 bg-white z-10">
